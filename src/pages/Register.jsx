...
function Register() {
  ...
  const handleRegister = async (data) => {
    await api
      ...
      .catch((err) => {
        if ((err.response.data.message = 'erorr: Gagal membuat akun, Email sudah terdaftar')) {
          toast({
    ...
    <Box minH={'100%'}>
      <Box>
        ...
        <Flex height={{ base: 'auto', md: 'auto', lg: 'full' }} direction={{ base: 'column', md: 'row', lg: 'row' }}>
          ...
          <Box mx={{ base: 'auto', lg: '0' }} width={{ base: '350px', sm: '500px', md: '700px', lg: '700px' }} px={{ base: '10', lg: '24' }} pb={{ base: '16' }}>
              ...
              <Box className="text-start w-full">
                <form>
                  <FormControl isInvalid={errors.username}>
                    <Input {...register('nama')} placeholder="Masukkan username" type={'text'} name="nama" />
                    <Text color={'red'}>{errors.username?.message}</Text>
                  </FormControl>
                  <br />
                  <FormControl isInvalid={errors.email}>
                    <Input {...register('email')} placeholder="Masukkan email" type={'email'} name={'email'} />
                    <Text color={'red'}>{errors.email?.message}</Text>
                    <br />
                  </FormControl>
                  <FormControl isInvalid={errors.nik}>
                    <Input {...register('nik')} type={'number'} name={'nik'} placeholder="Masukkan NIK" />
                    <Text color={'red'}>{errors.nik?.message}</Text>
                    <br />
                  </FormControl>
                  <FormControl isInvalid={errors.no_kk}>
                    <Input name="no_kk" {...register('no_kk')} type={'number'} placeholder="Masukkan No. Kartu Keluarga" />
                    <Text color={'red'}>{errors.no_kk?.message}</Text>
                    <br />
                  </FormControl>
                  <FormControl isInvalid={errors.nomorhape}>
                    <Input type="number" {...register('nomorhape')} placeholder="Masukkan nomor hp" name="no_telpon" />
                    <Text color="red">{errors.nomorhape?.message}</Text>
                  </FormControl>
                  <br />
                  <FormControl isInvalid={errors.kata_sandi}>
                    <InputGroup>
                      <Input type={show ? 'text' : 'password'} {...register('kata_sandi')} placeholder="Masukkan password" name="kata_sandi" onInput={onShowPassword} />
                      {passwordType && <InputRightElement>{show ? <ViewOffIcon onClick={showPassword} cursor={'pointer'} /> : <ViewIcon onClick={showPassword} cursor={'pointer'} />}</InputRightElement>}
                    </InputGroup>
                  </FormControl>
            ...
            <Box mt={5}>
              <Button colorScheme="white" color="#000000" variant="solid" border="2px" width="100%">
            ...
            <Box>
              <Center>
                <Text mt="5">
                  ...
                  <Link color="alta.primary" href="#" fontWeight="semibold" onClick={() => navigate('/login')}>
                    Login
                  </Link>
                </Text>
              </Center>
