...
const PatientRegistrationPage = () => {
  ...
  const getDetailStaff = async () => {
    await api
      ...
      .then((response) => {
        ...
        setLoading(false);
        ngambilDaftarBedDiHospital(token, response.data.data.hospital_id, currentPage);
      })
      .catch((err) => {
        toast({
          position: 'top',
  ...
  const ngambilDaftarBedDiHospital = async (token, hospitalId, currentPage) => {
    await api.ngambilBedYangKedaftarDiHospital(token, hospitalId, currentPage).then((response) => {
    ...
    <>
      {loading ? (
        ...
        <LayoutAdmin activeMenu={'patient'}>
          <HeadAdmin title={'Manajemen Pemesanan Kamar'} showFilter={'none'} showSearch={'none'} showAdd={'none'} />
                        ...
                        <Tr>
                          ...
                          <Td>
                            <ButtonGroup gap="4">
                              ...
                              <Button bg="transparent" border="1px" borderColor={'#E0E0E0'} onClick={() => hapusBedRegistrasi(token, bed.id)}>
                          ...
                          <Td textAlign={'center'}>
                            {bed.status_pembayaran === 'settlement' ? (
                              <Badge colorScheme="green">{bed.status_pembayaran}</Badge>
                            ) : bed.status_pembayaran === 'pending' ? (
                              <Badge colorScheme={'red'}>{bed.status_pembayaran}</Badge>
                          ...
                          <Td textAlign={'center'} color={'blue'}>
                            <Link hrefLang={bed.link_pembayaran}>Link Pembayaran</Link>
                          </Td>
              ...
              <>
                ...
                <Box mt={5}>
                  <FormControl>
                    ...
                    <Input value={selectedBed?.status_pasien} id={'status_pasien'} onChange={(e) => handlerChangeInput(e)} />
                  </FormControl>
                  <FormControl>
                    <FormLabel>Biaya Registrasi</FormLabel>
                    <Input value={selectedBed?.biaya_registrasi} id={'biaya_registrasi'} type={'number'} onChange={(e) => handlerChangeInput(e)} />
                  </FormControl>
                  <FormControl>
                    <FormLabel>Status Pembayaran:</FormLabel>
                    <Select id={'status_pembayaran'} value={selectedBed?.status_pembayaran} onChange={(e) => handlerChangeInput(e)}>
                      <option value={'settlement'}>settlement</option>
                      <option value={'pending'}>pending</option>
                      <option value={'settlement via BPJS'}>settlement via BPJS</option>
                  ...
                  <FormControl>
                    ...
                    <Input value={selectedBed?.bed_id} id={'bed_id'} onChange={(e) => handlerChangeInput(e)} />
                  </FormControl>
                  <Box textAlign={'end'} mt={5}>
                    <Button onClick={() => onSubmitEdited(selectedBed)} bg="#3AB8FF" color={'white'} fontSize={'14px'} fontWeight={'700'} width={'150px'} height={'50px'} _hover={{ bg: 'alta.primary' }}>
                      Perbaharui Data
                    </Button>
                  </Box>
                </Box>
