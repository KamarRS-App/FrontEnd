import { Box, Button, FormControl, FormErrorMessage, FormLabel, HStack, Image, Input, Stack, Text, useToast, Link } from '@chakra-ui/react';
    ...
    <Stack minWidth={'full'} minHeight={'100vh'} maxHeight={'100vh'} bg={'#FAFAFA'} p={{ base: '5', md: '10' }}>
      ...
      <HStack justify={'center'} alignItems="center">
        <Box maxWidth={'500px'} maxHeight={'650px'} bg={'white'} border={'1px'} borderRadius={'2xl'} shadow={'lg'} width={'full'} height={'full'} borderColor={'#E0E0E0'} px={{ base: '10', md: '80px' }} py={{ base: '10', md: '70px' }}>
          <Text color={'#1FA8F6'} fontSize={{ base: '26px', md: '28px' }} fontWeight={'600'}>
            Login as Super Admin
          </Text>
          <form onSubmit={handleSubmit(onSubmit)}>
            <FormControl mt={'8'} isInvalid={errors.email}>
              <FormLabel fontSize={'14px'} color={'#695C5C'}>
                Email
              </FormLabel>
              <Input placeholder="email@mail.com" id="email" type="email" border={'1px'} borderColor={'#00000066'} height={'50px'} borderRadius={'xl'} _placeholder={{ color: '#000000B2' }} {...loginAdmin('email')} />
              {errors.email && <FormErrorMessage>{errors.email.message}</FormErrorMessage>}
            </FormControl>
            <FormControl mt={'5'} isInvalid={errors.kata_sandi}>
              <FormLabel fontSize={'14px'} color={'#695C5C'}>
                Password
              </FormLabel>
              <Stack position={'relative'}>
                ...
                />
                {show && (
                  <Box position={'absolute'} right={'4'} bottom={'4'} cursor={'pointer'} onClick={() => setShowPassword(!showPassword)}>
            ...
            <Box align="end">
              <Text fontWeight={'500'} fontSize={'15px'} mt={5}>
                <Link color="#828282" href="#" fontWeight={400} onClick={() => navigate('/')}>
                  Kembali ke Beranda
                </Link>
              </Text>
            </Box>
