...
const RoomPage = () => {
  ...
  const { isOpen: isOpenModalEdit, onOpen: onModalEditOpen, onClose: onCloseModalEdit } = useDisclosure();
  const { isOpen: isOpenModalDelete, onOpen: onModalDeleteOpen, onClose: onCloseModalDelete } = useDisclosure();
        ...
        modalBody={
          <>
            ...
            <FormControl mt={4} isInvalid={errors.status}>
              ...
              <Select placeholder="Pilih Status Kamar Tidur" id="status" {...createHospitalBed('status')}>
        ...
        modalBody={
          <>
            <FormControl isInvalid={errorsUpdate.nama_tempat_tidur}>
              <FormLabel>Nama Tempat Tidur</FormLabel>
              <Input placeholder="Nama Tempat Tidur" id="nama_tempat_tidur" type="text" {...updateHospitalBed('nama_tempat_tidur')} />
              {errorsUpdate.nama_tempat_tidur && <FormErrorMessage>{errors.nama_tempat_tidur.message}</FormErrorMessage>}
            </FormControl>

            <FormControl isInvalid={errorsUpdate.ruangan}>
              <FormLabel>Ruangan</FormLabel>
              <Input placeholder="Ruangan" id="ruang" type="text" {...updateHospitalBed('ruangan')} />
              {errorsUpdate.ruangan && <FormErrorMessage>{errors.ruangan.message}</FormErrorMessage>}
            </FormControl>
            ...
            <FormControl mt={4} isInvalid={errorsUpdate.status}>
              <FormLabel>Status</FormLabel>
              <Select placeholder="Pilih Status Kamar Tidur" id="status" {...updateHospitalBed('status')}>
                <option>tersedia</option>
                <option>tidak tersedia</option>
              </Select>
              {errorsUpdate.status && <FormErrorMessage>{errors.status.message}</FormErrorMessage>}
            </FormControl>
          </>
        }
      />

      <PopupDelete isOpen={isOpenModalDelete} onClose={onCloseModalDelete} onDelete={() => onDeleteHandler(bedId)} modalBody={'Apakah anda yakin menghapus ruangan ini?'} modalTitle={'Hapus Ruangan'} delete_name={'Hapus Ruangan'} />
    </LayoutAdmin>
  );
};
