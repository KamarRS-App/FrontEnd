...
const PoliclinicPages = () => {
  ...
  const getPoliclinicByHospital = async () => {
    await api
      .getAllPoliclinics(token, staff.hospital_id)
      .then((response) => {
  ...
  const updatePoliclinicHandler = async (data) => {
    await api
      .updatePoliclinic(token, policlinicId, data)
      .then((response) => {
        toast({
          position: 'top',
  ...
  const getPoliclinicById = async () => {
    await api
      .getPoliclinicById(token, policlinicId)
      .then((response) => {
        const data = response.data.data;
  ...
  const deletePoliclinicHandler = async () => {
    await api
      .DeletePoliclinic(token, policlinicId)
      .then((response) => {
        toast({
          position: 'top',
  ...
  const createDoctorHandler = async (data) => {
    await axios
      .post(`https://rawatinap.online/doctors`, data, {
        headers: {
  ...
  const {
    register: updatePoliclinic,
    handleSubmit: submitUpdatePoliclinic,
  ...
  const {
    register: updateDoctor,
    handleSubmit: handleUpdateDoctor,
    formState: { errors: errorsUpdateDoctor },
    setValue: setValueUpdateDoctor,
  } = useForm({
    mode: 'onTouched',
    reValidateMode: 'onSubmit',
    resolver: yupResolver(schemaDoctor),
    ...
    <>
      ...
      {!loading && (
        <LayoutAdmin activeMenu={'poli'}>
          ...
          <Box mt={'5'} p={{ base: '2', sm: '5', md: '10' }} bg="white">
            {newDoctors.length !== 0 && (
              <Box width={'full'} textAlign={'end'}>
                <Button onClick={onOpenAddDoctor} bg="#3AB8FF" color={'white'} fontSize={'14px'} fontWeight={'700'} height={'50px'} _hover={{ bg: 'alta.primary' }}>
                  Tambah Data Dokter
                </Button>
              </Box>
            )}
                ...
                newDoctors.length === 0 ? (
                  <Tr>
                    <Td colSpan={'6'} textAlign={'center'}>
                      <Text fontWeight={'500'} color={'#ADB8CC'} mb={'5'}>
                        Data Belum Ada
                      </Text>
                      <Button onClick={onOpenAddDoctor} bg="#3AB8FF" color={'white'} fontSize={'14px'} fontWeight={'700'} height={'50px'} _hover={{ bg: 'alta.primary' }}>
                        Tambah Data Dokter
                  ...
                  newDoctors.map((doctor, index) => (
                    <Tr key={index}>
                      ...
                      <Td textAlign="center">
                        <ButtonGroup gap="4">
                          <Button onClick={() => handleEditeDoctor(doctor.id)} bg="transparent" border="1px" borderColor={'#E0E0E0'}>
                            <MdModeEdit />
                          </Button>
                          <Button onClick={() => handleDeleteDoctor(doctor.id)} bg="transparent" border="1px" borderColor={'#E0E0E0'}>
          ...
          <PopupDelete deletet_name={'Hapus Poliklinik'} modalBody={'Apakah kamu yakin menghapus Poliklinik?'} modalTitle={'Hapus Poliklinik'} isOpen={isOpenModalDelete} onClose={onCloseModalDelete} onDelete={() => onDeleteClicked()} />
            ...
            modalBody={
              <>
                <FormControl isInvalid={errorsDoctor.nama}>
                  ...
                  <Input {...registerDoctor('nama')} placeholder="Nama Dokter" id="name" type="text" name="nama" />
                  {errorsDoctor.nama && <FormErrorMessage>{errorsDoctor.nama.message}</FormErrorMessage>}
                </FormControl>

                <FormControl mt={4} isInvalid={errorsDoctor.email}>
                  <FormLabel>Email</FormLabel>
                  <Input {...registerDoctor('email')} name="email" placeholder="Email Dokter" type={'email'} id="email" />
                  {errorsDoctor.email && <FormErrorMessage>{errorsDoctor.email.message}</FormErrorMessage>}
                </FormControl>

                <FormControl mt={4} isInvalid={errorsDoctor.no_telpon}>
                  <FormLabel>No Telpon</FormLabel>
                  <Input {...registerDoctor('no_telpon')} name="no_telpon" placeholder="Nomor Telephone Dokter" type={'number'} id="no_telpon" />
                  {errorsDoctor.no_telpon && <FormErrorMessage>{errorsDoctor.no_telpon.message}</FormErrorMessage>}
                </FormControl>
            ...
            modalBody={
              <>
                <FormControl isInvalid={errorsUpdateDoctor.nama}>
                  <FormLabel>Nama</FormLabel>
                  <Input {...updateDoctor('nama')} placeholder="Nama Dokter" id="name" type="text" name="nama" />
                  {errorsUpdateDoctor.nama && <FormErrorMessage>{errorsUpdateDoctor.nama.message}</FormErrorMessage>}
                </FormControl>

                <FormControl mt={4} isInvalid={errorsUpdateDoctor.email}>
                  <FormLabel>Email</FormLabel>
                  <Input {...updateDoctor('email')} name="email" placeholder="Email Dokter" type={'email'} id="email" />
                  {errorsUpdateDoctor.email && <FormErrorMessage>{errorsUpdateDoctor.email.message}</FormErrorMessage>}
                </FormControl>

                <FormControl mt={4} isInvalid={errorsUpdateDoctor.no_telpon}>
                  <FormLabel>No Telpon</FormLabel>
                  <Input {...updateDoctor('no_telpon')} name="no_telpon" placeholder="Nomor Telephone Dokter" type={'number'} id="no_telpon" />
                  {errorsUpdateDoctor.no_telpon && <FormErrorMessage>{errorsUpdateDoctor.no_telpon.message}</FormErrorMessage>}
                </FormControl>
