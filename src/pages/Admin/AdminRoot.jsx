...
const AdminRoot = () => {
  ...
  const [initialValueUpdate, setInitialValueUpdate] = useState(initialValuesUpdate);

  const schema = Yup.object().shape({
    nama: Yup.string().required('Nama tidak boleh kosong'),
    email: Yup.string().email('Format Email salah').required('Email tidak boleh kosong'),
    kata_sandi: Yup.string().min(6, 'Password tidak boleh kurang dari 6 karakter').max(32, 'Password tidak boleh lebih dari 32 karakter').required('Password tidak boleh kosong'),
          ...
          adminStaff.length !== 0 ? (
            adminStaff.map((data, index) => (
              <Tr key={index + 1}>
                ...
                <Td textAlign={'center'}>
                  <ButtonGroup gap="4">
                    <Button onClick={() => onEditHandler(data.id)} bg="transparent" border="1px" borderColor={'#E0E0E0'}>
                      <MdModeEdit />
                    </Button>
          ...
          <>
            ...
            <FormControl mt={'4'} isInvalid={errors.kata_sandi}>
              ...
              <Stack position={'relative'}>
                <Input placeholder="password" id="kata_sandi" type={showPassword ? 'text' : 'password'} onInput={onShowPassword} {...createAdmin('kata_sandi')} />
                {show && (
