...
function CariSpesialis() {
  ...
  const getPractices = async (id, practiceDate) => {
    await api.getAllDailyPractices(token, id)
      .then((response) => {
  ...
  const getPoliclinics = async (hospital_id) => {
    await api.getAllPoliclinics(token, hospital_id)
      .then((response) => {
        const data = response.data.data;
        setPoliclinics(data);
      })
  ...
  const getAllHospitalsHandler = async () => {
    await api.getAllHospitals(token)
      .then((response) => {
        const data = response.data.data;
        setHospitals(data);
      })
            ...
            <Stack spacing={3}>
              <Box>
                ...
                <Select placeholder="-- Pilih provinsi --" onChange={(e) => setKabupaten(e.target.value)}>
                  {provinsi?.map((prov) => {
                    return <option value={prov.id} key={prov.id}>{prov.nama}</option>;
                  })}
                </Select>
              </Box>
              <Box>
                ...
                <Select placeholder="-- Pilih kabupaten/kota --">
                  {listKabupaten?.map((kota) => {
                    return <option value={kota.nama} key={kota.id}>{kota.nama}</option>;
                  })}
                </Select>
              </Box>
              <Box>
              ...
              <Box color="#000000">
                ...
                <Select onChange={(e) => handlerPoliclinics(e.target.value)} placeholder="-- Pilih Poliklinik --">
                  {policlinics?.map((data) => {
                    return <option value={data.id} key={data.id}>{data.nama_poli}</option>;
                  })}
                </Select>
              </Box>
              <Box>
